...
https.get(url, (res) => {
    ...
    res.on('end', async () => {
        ...
        for (const doc of (provs.documents || [])) {
            ...
            const provSheet = sheetRows.filter(r => {
                const p1 = extraerPalabrasClave(provId).join(' ');
                const p2 = extraerPalabrasClave(r.proveedor).join(' ');
                return p1.includes(p2) || p2.includes(p1);
            });

            if (provSheet.length === 0) continue;
        ...
        console.log(`\n🎉 Total productos analizados: ${totalProds} | Coincidencias encontradas: ${totalMatched} (${(totalMatched/totalProds*100).toFixed(1)}%)`);
    });
});
